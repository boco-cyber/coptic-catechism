...
V2_OCR_DIR = os.path.join(OUT_DIR, "verification", "ocr_v2")
BOOK_RANGES = {
...
def extract_book(book_number, index):
    ...
    path = os.path.join(V2_OCR_DIR, f"ocr_book{book_number}.txt")
...
def main():
    ...
    output_path = os.path.join(OUT_DIR, "verification", "catechism_qa_ar_v2.json")
    if args.check:
    ...
    print(f"DIAGNOSTIC: {len(output) - len(missing)}/{len(output)} complete records: "
          f"{total_aligned} headings aligned, {len(missing)} incomplete")
